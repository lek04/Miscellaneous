...
{//double f[7] = { 10, 50, 40, 20, 50, 40, 30 };
//assert(countIsIn(g, 3, f, 7) == 1);

	if (n1 < 0 || n1 == 0) { //will break out whenever a1 decrements to zero
		return 1;
	}
	if (n2 < 1 || n2 == 1) { //will break out whenever a2 decrements to zero before n1 does
		if (a1[0] == a2[0]) {
			if (n1 == 1) {
				return 1;
			}
	...
	if (a1[0] == a2[0]) {
		int x = countIsIn(a1 + 1, n1 - 1, a2 + 1, n2 - 1);
		int y = countIsIn(a1, n1, a2 + 1, n2 - 1);
		return x + y;
	}
	else {
